...
export default defineConfig({
  server: {
    ...
    proxy: {
      ...
      // (Vercel sandbox deployment), so POST /deck and POST /deck/apply_ops
      // must NOT be forwarded — otherwise anyone with the URL could mutate
      // the deck without authentication.
      "/deck": {
        target: BACKEND_URL,
        changeOrigin: true,
        bypass(req) {
