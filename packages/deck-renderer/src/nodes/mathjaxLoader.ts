...
//
// The two halves of this object MUST stay in sync for the FIVE features the
// menu back-writes onto document options (enumerated at line 36 above:
// enrichment / speech / braille / complexity / explorer). Each of those
// top-level `enableX: false` flags has a corresponding
// `menuOptions.settings.x: false` counterpart and editing only one will
// silently re-enable the feature.
//
// AssistiveMml is the exception: in MathJax v4 it's a pure output
// post-processor gated only by `menuOptions.settings.assistiveMml`. There is
// NO top-level `enableAssistiveMml` in v4's option registry — passing one
// triggers v4's strict option validator with `Invalid option ... (no
// default value)`. The asymmetry is intentional; do not pair a top-level
// twin onto assistiveMml when upgrading.
...
export function loadMathJax(): Promise<MathJaxAPI> {
  ...
  const pending = new Promise<MathJaxAPI>((resolve, reject) => {
    ...
    w.MathJax = {
      ...
      // Deck objects are independently positioned/resized. MathJax's SVG
      // output defaults to automatic inline line-breaking (`inline: true`),
      // which can split a short inline formula like `x^2 + y^2` across rows
      // when the editor box is narrow during typeset. Keep formulas atomic;
      // editor auto-fit decides the box size after SVG measurement.
