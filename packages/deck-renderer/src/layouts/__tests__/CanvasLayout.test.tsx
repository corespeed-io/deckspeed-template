...
describe("CanvasLayout", () => {
  ...
  it("exposes the canvas root for editor pointer handling and overlays", () => {
    const nodes: DeckNodeT[] = [
      {
        id: "t1",
        type: "Text",
        pos: { mode: "canvas", unit: "mm", x: 10, y: 20, w: 50, h: 8 },
        props: { plain: "Pos" },
      },
    ];
    const byId = new Map(nodes.map((n) => [n.id, n]));
    ...
    const { container, getByTestId } = render(
      <CanvasLayout
        ...
        byId={byId}
        slideId="showcase"
        canvasPointerHandlers={{ onPointerDown }}
        canvasOverlay={<div data-testid="canvas-overlay" />}
      />,
    );

    const canvas = container.querySelector(
      '[data-canvas-layout="showcase"]',
    ) as HTMLElement;
    expect(canvas).not.toBeNull();
    fireEvent.pointerDown(canvas);
    expect(onPointerDown).toHaveBeenCalledTimes(1);
    expect(getByTestId("canvas-overlay")).not.toBeNull();
  });
